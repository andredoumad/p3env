!define PRODUCT_NAME "List App (PyQt5)"
!define PRODUCT_VERSION "1.0"
!define PY_VERSION "3.5.1"
!define PY_MAJOR_VERSION "3.5"
!define BITNESS "64"
!define ARCH_TAG ".amd64"
!define INSTALLER_NAME "List_App_(PyQt5)_1.0.exe"
...
!define MULTIUSER_INSTALLMODE_INSTDIR "List App (PyQt5)"
...
Section "!${PRODUCT_NAME}" sec_app
  ...
  StrCmp $MultiUser.InstallMode CurrentUser 0 +3
    ...
    SetFileAttributes "$INSTDIR\${USER_INSTALL_MARKER}" HIDDEN
      ...
      ; Install files
    SetOutPath "$INSTDIR"
      File "List_App_(PyQt5).launch.pyw"
      File "glossyorb.ico"
  ...
  SetOutPath "%HOMEDRIVE%\%HOMEPATH%"
    CreateShortCut "$SMPROGRAMS\List App (PyQt5).lnk" "$INSTDIR\Python\pythonw.exe" \
      '"$INSTDIR\List_App_(PyQt5).launch.pyw"' "$INSTDIR\glossyorb.ico"
  SetOutPath "$INSTDIR"
...
Section "Uninstall"
  ...
  ; Uninstall files
    Delete "$INSTDIR\List_App_(PyQt5).launch.pyw"
    Delete "$INSTDIR\glossyorb.ico"
  ; Uninstall directories
    RMDir /r "$INSTDIR\Python"

  ; Uninstall shortcuts
      Delete "$SMPROGRAMS\List App (PyQt5).lnk"
  RMDir $INSTDIR
