!define PRODUCT_NAME "Guess the Number"
!define PRODUCT_VERSION "1.0"
!define PY_VERSION "3.6.3"
!define PY_MAJOR_VERSION "3.6"
!define BITNESS "64"
!define ARCH_TAG ".amd64"
!define INSTALLER_NAME "Guess_the_Number_1.0.exe"
...
!define MULTIUSER_INSTALLMODE_INSTDIR "Guess the Number"
...
Section "!${PRODUCT_NAME}" sec_app
  ...
  StrCmp $MultiUser.InstallMode CurrentUser 0 +3
    ...
    SetFileAttributes "$INSTDIR\${USER_INSTALL_MARKER}" HIDDEN
      ...
      ; Install files
    SetOutPath "$INSTDIR"
      File "Guess_the_Number.launch.py"
      File "glossyorb.ico"
      File "_system_path.py"
      File "_assemble_launchers.py"
  ...
  SetOutPath "%HOMEDRIVE%\%HOMEPATH%"
    CreateShortCut "$SMPROGRAMS\Guess the Number.lnk" "$INSTDIR\Python\python.exe" \
      '"$INSTDIR\Guess_the_Number.launch.py"' "$INSTDIR\glossyorb.ico"
  SetOutPath "$INSTDIR"

    DetailPrint "Setting up command-line launchers..."
    nsExec::ExecToLog '"$INSTDIR\Python\python" -Es "$INSTDIR\_assemble_launchers.py" "$INSTDIR\bin"'
...
Section "Uninstall"
  ...
  ; Uninstall files
    Delete "$INSTDIR\Guess_the_Number.launch.py"
    Delete "$INSTDIR\glossyorb.ico"
    Delete "$INSTDIR\_system_path.py"
    Delete "$INSTDIR\_assemble_launchers.py"
  ; Uninstall directories
    RMDir /r "$INSTDIR\Python"
    RMDir /r "$INSTDIR\bin"

  ; Uninstall shortcuts
      Delete "$SMPROGRAMS\Guess the Number.lnk"
  RMDir $INSTDIR
